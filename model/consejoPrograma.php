...
/*
	...
	$resultado = mysqli_query($mysqli,$consulta); $j=0;
  		...
  		*/
  	$consulta = "SELECT candidato.id_candidato, candidato.foto, candidato.id_organo, candidato.numero, usuario.nombre1, usuario.nombre2, usuario.apellido1, usuario.apellido2 FROM usuario INNER JOIN candidato ON candidato.id_candidato = usuario.codigo WHERE candidato.id_organo = '4' ORDER BY usuario.nombre1";
  	$resultado = mysqli_query($mysqli,$consulta); $j=0;
  		while($mostrar=mysqli_fetch_object($resultado)){
    		$idcandidato[$j] = $mostrar->id_candidato;
    		$foto[$j] = $mostrar->foto;
    ...
    $consulta2 = "SELECT nombre FROM organo WHERE id_organo = '4'";
